const db = require('./db');
const Joi = require('joi');

const pedidoSchema = Joi.object({
    id: Joi.string().required(),
    forma_pagamento: Joi.string().required(),
    qtde_itens: Joi.string().required(),
    valor_total: Joi.string().required(),
    cpf: Joi.string().length(11).required(),
    id_entregador: Joi.string().required(),
});

exports.listarPedido = (req, res) => {
    db.query('SELECT * FROM pedido', (err, result) => {
        if (err) {
            console.error('Erro ao buscar pedido: ', err);
            res.status(500).json({ error: 'Erro interno do servidor' });
            return;
        }
        res.json(result);
    });
};

exports.buscarPedido = (req, res) => {
    const { id } = req.params;
    db.query('SELECT * FROM pedido WHERE id = ?', id, (err, result) => {
        if (err) {
            console.error('Erro ao buscar pedido:', err);
            res.status(500).jsaon({ error: 'Erro interno do servidor' });
            return;
        }
        if (result.length === 0) {
            res.status(404).json({ error: 'Driver not found' });
            return;
        }
        res.json(result[0]);
    });
};

exports.adicionarPedido = (req, res) => {
    const { forma_pagamento, qtde_itens, valor_total, cpf, id_entregador } = req.body;
    const { error } = pedidoSchemaSchema.validate({ forma_pagamento, qtde_itens, valor_total, cpf, id_entregador });
    if (error) {
        res.status(400).json({ error: 'Dados de pedido inválidos' });
        return;
    }
    const novoPedido = { forma_pagamento, qtde_itens, valor_total, cpf, id_entregador };
    db.query('INSERT INTO pedido SET ?', novoPedido, (err, result) => {
        if (err) {
            console.error('Erro ao adicionar pedido', err);
            res.status(500).json({ error: 'Erro interno do servidor' });
            return;
        }
        res.jason({ message: 'Pedido adicionado com sucesso' });
    });
};

exports.atualizarPedido = (req, res) => {
    const { id } = req.params;
    const { forma_pagamento, qtde_itens, valor_total, cpf, id_entregador } = req.body;
    const { error } = pedidoSchema.validate({ forma_pagamento, qtde_itens, valor_total, cpf, id_entregador });
    if (error) {
        res.status(400).json({ error: 'Dados de pedido inválidos' });
        return;
    }
    const pedidoAtualizado = { forma_pagamento, qtde_itens, valor_total, cpf, id_entregador };
    db.query('UPDATE pedido SET ? WHERE id = ?', [pedidoAtualizado, id], (err, result) => {
        if (err) {
            console.error('Erro ao atualizar pedido:', err);
            res.status(500).json({ error: 'Erro interno do servidor' });
            return;
        }
        res.json({ message: 'pedido atualizado com sucesso' });
    });
};

exports.deletarPedido = (req, res) => {
    const { id } = req.params;
    db.query('DELETE FROM pedido WHERE cpf = ?', id, (err, result) => {
        if (err) {
            console.error('Erro ao deletar pedido:', err);
            res.status(500).json({ error: 'Erro interno do servidor' });
            return;
        }
        res.json({ message: 'pedido deletado com sucesso' });
    });
};